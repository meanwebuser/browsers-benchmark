...
class NodePlaywrightEngine(BrowserEngine):
    def __init__(
            self,
            name: str = "node-playwright-chromium_headless",
    ...
    async def start(self) -> None:
        ...
        worker_path = os.path.join(os.path.dirname(__file__), "node_playwright_worker.js")
        if not os.path.exists(worker_path):
            raise FileNotFoundError(f"{self.name}: worker file is missing: {worker_path}")
        ...
        try:
            ...
            await self._send_command(
                "start",
                {
                    ...
                    "useSystemChrome": self.use_system_chrome,
                    "pageLoadTimeoutMs": settings.browser.page_load_timeout_s * 1000,
                    "actionTimeoutMs": settings.browser.action_timeout_s * 1000,
                },
                timeout_s=max(15, settings.browser.page_load_timeout_s + 5),
            )
        except Exception:
            await self.stop()
            raise
    ...
    async def _send_command(self, command: str, payload: Dict[str, Any], timeout_s: int) -> Dict[str, Any]:
        async with self._io_lock:
            ...
            if not self._worker.stdin or not self._worker.stdout:
                raise RuntimeError(f"{self.name}: worker pipes are unavailable")
    ...
    async def _read_response(self, expected_id: int) -> Dict[str, Any]:
        ...
        while True:
            ...
            if not raw_line:
                ...
                raise RuntimeError(
                    f"{self.name}: worker closed stdout unexpectedly."
